...
public class BookServiceImpl implements BookService{
    ...
    public List<BookExpanded> getBooks() {
        ...
        for (Book tempBook: bookList){
            ...
            if (tempBook.getBookAuthorList().size() > 0)
                bookExpanded.setAuthorFirstName(tempBook.getBookAuthorList().get(0).getAuthorsFirstName());
            else
                bookExpanded.setAuthorFirstName(tempBook.getAuthorFirstName());
            if (tempBook.getBookAuthorList().size() > 0)
                bookExpanded.setAuthorLastName(tempBook.getBookAuthorList().get(0).getAuthorsLastName());
            else
            ...
            bookExpanded.setContactEmail(tempBook.getContactEmail());
